...
Database::insertTripleByGroup(const TripleWithObjType* _triple, TYPE_TRIPLE_NUM _triple_num, vector<unsigned>* _vertices, vector<unsigned>* _predicates)
{
	...
	qsort(id_tuples, valid_num, sizeof(int*), Util::_spo_cmp);
	for(int i = 0; i < valid_num; ++i)
	{
		(this->kvstore)->updateInsert_s2values(id_tuples[i][0], id_tuples[i][1], id_tuples[i][2]);
	}

	qsort(id_tuples, valid_num, sizeof(int*), Util::_ops_cmp);
	for(int i = 0; i < valid_num; ++i)
	{
		(this->kvstore)->updateInsert_o2values(id_tuples[i][0], id_tuples[i][1], id_tuples[i][2]);
	}

	qsort(id_tuples, valid_num, sizeof(int*), Util::_pso_cmp);
	for(int i = 0; i < valid_num; ++i)
	{
		(this->kvstore)->updateInsert_p2values(id_tuples[i][0], id_tuples[i][1], id_tuples[i][2]);
	}
